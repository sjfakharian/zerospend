...
HTTP 429 is classified separately as `local_rate_limited`. It is temporary capacity evidence, not permanent model invalidation: the official/free evidence remains intact, the model is not production eligible without a successful probe, and discovery tries the next distinct candidate up to the same three-candidate limit. If every bounded candidate is rate-limited, provider status is `TEMPORARY_CAPACITY` with `temporary_free_capacity_unavailable`. `Retry-After` is parsed for diagnostics but does not cause discovery to sleep or retry the same model.
