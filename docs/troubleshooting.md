...
- **9Router installed but offline:** run `9router --tray --host 127.0.0.1 --port 20128`, verify the listener is loopback-only, then use the reviewed user LaunchAgent template if persistence is needed. `--no-browser` does not background the process. Direct OpenRouter, NVIDIA, TokenHarbor, and APINex routes remain available without it.
